...
def post_message_to_slack_channel(message):
    ...
    if not response.ok:
        logger.error(
            'Posting message on Slack failed with status code {0}. Please check the errors: "{1}"'.format(
...
def trigger_knowledge_store_broadcast_activity(
    knowledge_store_object_id, broadcast_channels=[]
):
    """
